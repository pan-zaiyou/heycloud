...
import { useGetGuestConfigQuery, useSendEmailVerifyMutation, useCheckEmailRegisteredQuery } from "@/store/services/api";
...
const SendMailButton: React.FC<SendMailButtonProps> = ({ email }) => {
  ...
  const { enqueueSnackbar } = useSnackbar();
  const { t } = useTranslation();
  const { data: emailCheckData, isLoading: isCheckingEmail } = useCheckEmailRegisteredQuery({ email }, { skip: !email });

  const handleSendEmailCode = () => {
    if (isCheckingEmail) return;

    if (emailCheckData?.is_registered) {
      if (siteConfig?.is_recaptcha === 1) {
        // Use reCAPTCHA if enabled
        setOpen(true);
      } else {
        // Send email without reCAPTCHA
        sendMail({ email })
          .unwrap()
          .then(() => {
            enqueueSnackbar(t("auth.captcha.success"), { variant: "success" });
      ...
      enqueueSnackbar(t("auth.email_not_registered"), { variant: "error" });
    }
  };
          ...
          <DialogContent>
            <ReCaptcha
              ...
              onChange={(token: string | null) => {
                ...
                sendMail({ email, recaptcha_data: token! })
                  .unwrap()
                  .then(() => {
                    enqueueSnackbar(t("auth.captcha.success"), { variant: "success" });
                    ReactGA.event("send_email_verify", {
